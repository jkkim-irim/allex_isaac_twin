...
class SimStateLogger:
    ...
    def _write_showcase_csv(self, out_dir: str, time_arr: np.ndarray) -> None:
        ...
        # ---- header ----
        # pos_/torque_ 컬럼은 showcase_reader 가 short-form (lowercase, no `_Joint`)
        # 으로 reverse-map 하므로 joint_to_csv_short 로 emit 한다.
        from .joint_name_map import joint_to_csv_short
        short_names = [joint_to_csv_short(n.split("/")[-1]) for n in self._torque_names]
        cols: list[str] = ["time"]
        cols += [f"pos_{s}"    for s in short_names]
        cols += [f"torque_{s}" for s in short_names]
        pair_names: list[str] = []
