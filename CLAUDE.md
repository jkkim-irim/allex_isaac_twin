...
## USD layer 규약 (visualizer / 임시 prim)

ALLEX twin 의 viz prim 들은 **stage session layer** 에 authored — root layer (=ALLEX.usd 의 sublayer) 에 영구 흔적 남기지 않기 위해서. 새 prim/attribute 를 추가할 때 layer 선택을 잘못하면 **layer composition strength** 때문에 의도와 정반대로 동작할 수 있다.

**기본 원칙: viz/임시 prim 의 *모든* edit (DefinePrim · 어트리뷰트 · visibility · scale · displayColor) 은 session layer 에서 한다.**

- **Layer strength**: `session > root sublayers` (LIVRPS 의 session 가 가장 높음). session 에 authored 된 값은 root 의 같은 attribute 를 무시.
- **흔한 함정**: `_create_force_ref_xform` / `_create_ref_xform` 은 `Usd.EditContext(stage, stage.GetSessionLayer())` 안에서 prim 을 만든다. 이때 prim 은 session 에서 `visibility = inherited` 로 default authoring. 이후 다른 코드가 default edit target (root) 으로 `MakeInvisible` 만 호출하면, root 에는 `invisible` 박혀도 session 의 `inherited` 가 이겨서 prim 이 계속 visible. (force OFF 토글이 안 먹는 증상으로 나타났던 실제 버그.)
- **올바른 패턴**: 모든 viz 어트리뷰트 write 를 `with self._session_edit():` 컨텍스트로 감쌀 것 (`force_torque_visualizer.py::_session_edit`). `_set_visible` / `_apply_scale` / `_apply_force_*` / `set_custom_force_vector` 모두 이 컨텍스트 사용 중. 새 viz 메서드 추가할 때도 동일하게.
- **검증 팁**: visibility 토글이 안 먹는 것 같으면 USD Composer/Usdview 의 layer stack 패널에서 root 와 session 양쪽에 같은 attribute 가 박혀 있는지 확인. session 에 `inherited` / root 에 `invisible` 같이 split 돼 있으면 위 함정.
- **예외**: `set_camera_view`, `World/GroundPlane` 등 stage 의 영구 fixture 는 session 이 아니라 root 에 authored 해도 됨. session 은 lifetime 짧은 / 재시작 시 사라져도 되는 prim 전용.
