...
```json
{
  ...
  "ext_torque_triggers": {
    "ext_torque_arm_r":        [[33.0, 38.5]],
    "ext_torque_hand_l_thumb": "off"
  },
...
|---|---|---|
| `force_triggers` | sim active 또는 default visible (아래 표 참조) | 그 topic 만 hide |
...
real CSV header 의 `ext_torque_<topic_id>_x/y/z` 컬럼 prefix. 일반 패턴
`ext_torque_<topic_id>`. topic_id 는 `force_triggers` 와 동일 — `EXT_FORCE_TOPICS`
가 force/torque 토픽 매핑을 공유한다.

#### 등록된 모든 key (총 14, force 와 동일)

| JSON key                   | CSV 컬럼                            |
|----------------------------|-------------------------------------|
| `ext_torque_arm_l`         | `ext_torque_arm_l_{x,y,z}`          |
| `ext_torque_arm_r`         | `ext_torque_arm_r_{x,y,z}`          |
| `ext_torque_shoulder_l`    | `ext_torque_shoulder_l_{x,y,z}`     |
| `ext_torque_shoulder_r`    | `ext_torque_shoulder_r_{x,y,z}`     |
| `ext_torque_hand_l_thumb`  ~ `_little`  | (L 5종)                |
| `ext_torque_hand_r_thumb`  ~ `_little`  | (R 5종)                |
...
| `ext_torque_triggers[topic]` | 다른 topic key | 결과 게이트         |
|------------------------------|----------------|---------------------|
...
- Force arrow 게이팅: `csv_replayer.py::_push_real_force_routed`
...
- Trigger 파서:
  - `csv_replayer.py::_parse_prefixed_triggers` — force_triggers / ext_torque_triggers
...
- viz_config 상수 (per-joint ext_torque): `EXT_JOINT_TORQUE_GAIN_MULT`,
  `EXT_JOINT_TORQUE_COLOR_REAL/SIM`
